...
AppConfig loadConfig() {
  ...
  if (!configFile.is_open()) {
    logMessage("⚠️ config/app_config.json not found. Using default values.");
    return config;
  }
  try {
    json j;
...
json parseQPGS(const std::string &cleanResponse, const std::string &inverterId,
  ...
  int battery_total_charging =
      to_int(fields[15]); // <-- Required to prevent index shift
  ...
  j["config_max_ac_charger_current"] =
      to_int(fields[24]); // ← used in AC power calculation

  j["pv_total_input_current"] =
      pv_total_i; // ← This is kept (replaces "total_inv_pv_input_current")
  ...
  // 🔸 CALCULATION: ac_input_power_estimate = grid_voltage *
...
int main() {
  initLogger();
  logMessage("🚀 Starting axpert_monitor in continuous mode (reload config "
             "each cycle)...");
