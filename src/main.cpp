...
#include <sys/stat.h>
#include <sys/time.h> // Required for Alpine/musl
#include <thread>
#include <unistd.h>
#include <vector>
...
void createLogDir() {
  ...
  if (stat(LOG_DIR.c_str(), &info) != 0) {
    ...
    logMessage("📁 Log directory created: " + LOG_DIR);
  }
}
...
void rotateLogs() {
  ...
  if (logFiles.size() > MAX_LOG_FILES) {
    for (size_t i = MAX_LOG_FILES; i < logFiles.size(); ++i) {
      ...
      logMessage("🗑️  Old log deleted: " + logFiles[i]);
    }
  }
}
...
AppConfig loadConfig() {
  ...
  if (!configFile.is_open()) {
    logMessage(
        "⚠️ config/app_config.json not found. Using default values.");
    return config;
  }
  try {
    json j;
    ...
    logMessage("⚙️  Configuration loaded from config/app_config.json");
  } catch (const std::exception &e) {
    logMessage("⚠️ Error parsing config/app_config.json: " +
               std::string(e.what()));
  }
  return config;
}
...
void publishMQTT(struct mosquitto *mosq, const char *topic, const json &data) {
  ...
  if (ret != MOSQ_ERR_SUCCESS) {
    logMessage("❌ MQTT publish failed: " + std::string(topic));
  } else {
    logMessage("✅ Published (retain) to: " + std::string(topic));
  }
}
...
sendCommandAndGetCleanResponse(int sockfd,
  ...
  if (bytes <= 0)
    throw std::runtime_error("Timeout on receive");
...
json parseQPGS(const std::string &cleanResponse, const std::string &inverterId,
    ...
    if (fields.size() < 28) {
        throw std::runtime_error("Less than 28 fields in QPGS");
    }
    ...
    // --- PV calculations (fields 14, 25, 27) ---
    ...
    // 🔸 IMPORTANT: READ field[15] even if we don't store it
    int battery_total_charging = to_int(fields[15]); // <-- Required to prevent index shift
    // j["battery_total_all_inputs_charging_current"] = battery_total_charging; // NOT included
    ...
    j["config_max_ac_charger_current"] = to_int(fields[24]); // ← used in AC power calculation

    j["pv_total_input_current"] = pv_total_i; // ← This is kept (replaces "total_inv_pv_input_current")
    ...
    // 🔸 CALCULATION: battery_real_charge_current
    ...
    // 🔸 CALCULATION: battery_real_power = real_charge * battery_voltage
    ...
    // 🔸 CALCULATION: ac_input_power_estimate = grid_voltage * config_max_ac_charger_current
...
int main() {
  initLogger();
  logMessage("🚀 Starting axpert_monitor in continuous mode (reload config each cycle)...");

  // --- Start MQTT ---
  mosquitto_lib_init();
  struct mosquitto *mosq = mosquitto_new("axpert_monitor", true, nullptr);
  if (!mosq) {
    logMessage("❌ Error creating MQTT client");
    return 1;
  }
  ...
  while (true) {
    ...
    logMessage("🔄 New iteration (continuous mode)");

    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0) {
      logMessage("❌ Error creating TCP socket");
      std::this_thread::sleep_for(
          std::chrono::milliseconds(g_config.delay_between_cycles_ms));
      continue;
    }
    ...
    if (inet_pton(AF_INET, g_config.inverter1_tcp_ip.c_str(),
      ...
      logMessage("❌ Invalid inverter IP: " + g_config.inverter1_tcp_ip);
      close(sockfd);
      std::this_thread::sleep_for(
          std::chrono::milliseconds(g_config.delay_between_cycles_ms));
      continue;
    ...
    if (connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
      logMessage("❌ Failed to connect to TCP/serial converter at " +
    ...
    try {
      ...
      // 🔍 [OPTIONAL] Debug logs (you can comment them out if not needed)
      ...
      logMessage("⚠️ Error in cycle: " + std::string(e.what()));
    }
